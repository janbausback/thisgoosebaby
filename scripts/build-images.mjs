 ...
 *
 * Two screens are fed from here. The landing page (`/`) is the hero video and
 * its poster; the exhibition page (`/zwischentoene.html`) is the woven
 * background and the three drifting rugs.
 */
...
// enough, since Safari does not always draw it before the video has metadata.
// A plain <img> beneath the video is always there from first paint.
...
// so it is cropped to inside the rug's own woven edge instead.
...
const total = Object.values(patch).reduce(
  (sum, a) => sum + (a.widths ? Object.keys(a.widths).length : 1), 0
);
